...
export function Skills() {
  ...
  return (
    ...
    >
        ...
        {skillGroups.map((g, i) => (
          <button
            ...
            className={`relative rounded-full px-5 py-2.5 text-sm font-medium transition-colors duration-300 ${
              activeIdx === i ? "text-brand-foreground" : "text-foreground/60 hover:text-foreground"
            }`}
          >
            {activeIdx === i && (
              <motion.span
        ...
        >
          {active.items.map((item) => (
            <motion.li
              ...
              className="group relative overflow-hidden rounded-2xl border border-hairline bg-surface/50 px-6 py-5 backdrop-blur transition-colors duration-500 hover:border-brand/40 hover:bg-surface"
            >
      ...
      <div className="mt-20 space-y-3 md:mt-24">
